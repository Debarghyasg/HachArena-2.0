/**
 * Grahak Sathi — Post-Purchase Conversational Auditor (Otari Challenge, Case 1)
 * ───────────────────────────────────────────────────────────────────────────
...
function createAuditor({ llm, axios, fastapiUrl, budget, router, injection,
    ...
    async function handleMessage(p) {
        ...
        const imageName = p.imageName || null;  // uploaded filename (used by demo damage mode)
        const routing = [];
        ...
        if (!customerImage && !mkId) missing.push('a clear photo of the product showing the damage (or its MK-ID)');
        if (missing.length) {
        ...
        // Reading the MK-ID and assessing damage from the photo is a High-tier
        // (vision) task — budget-aware degradation. We only need High when there
        // is actually a photo to inspect; a typed MK-ID with no photo can't be
        // assessed for damage anyway and is handled cheaply downstream.
        const snap2  = await budget.snapshot(sessionId);
        const visDec = router.route('auditor_vision', { budget: snap2 });
        routing.push(visDec);

        if (visDec.tier !== 'high' && customerImage) {
        ...
        // Check the photo for damage AND match the MK-ID against this
        // transaction's checkout record. Refund only if broken AND matched.
        let rv = null;
        const t0 = Date.now();
        try {
        ...
        // Decision — refund only when the item is BROKEN *and* matches the
        // checkout record. Mismatch or intact item => no refund.
        const REVIEW_REASONS = ['lookup_error', 'unrecognized', 'condition_unknown'];
        let decision, reply;
        if (rv.refund_done) {
            decision = 'APPROVED';
            reply = `✅ ${rv.message} ${RETURN_POLICY_TEXT}`;
        } else if (REVIEW_REASONS.includes(rv.reason)) {
            decision = 'NEEDS_REVIEW';
            reply = rv.message;
        } else {
            // mkid_mismatch | not_in_user_history | item_intact | transaction_not_found
            decision = 'DENIED';
        ...
        return {
            ...
            verification: {
                ...
                damaged:        rv.damaged ?? null,
                intact:         rv.intact ?? null,
