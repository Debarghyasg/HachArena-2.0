...
const INTENTS = [
    ...
    },
    {
        intent: 'order_status',
        patterns: [
            // An order/delivery noun together with a status/tracking word (in either order).
            /\b(order|transaction|purchase|delivery|package|parcel|shipment|refund|return|claim)\b.*\b(status|track|tracking|where|update|progress|approved|processed|arriv|deliver|dispatch|shipp|ready|eta)\b/i,
            /\b(status|track|tracking|update|progress|eta)\b.*\b(order|transaction|purchase|delivery|package|parcel|shipment|refund|return|claim)\b/i,
            // "where's / wheres / where is / where's my order|delivery|stuff|item…" (contraction-tolerant).
            /\bwhere'?s?\b.*\b(order|delivery|package|parcel|shipment|stuff|item|purchase|refund|return)\b/i,
            // Explicit trackers, bare "order status", and "my order/delivery/…".
            /\b(track my|status of my|status on my|order status)\b/i,
            /\bmy (order|delivery|package|parcel|shipment)\b/i,
            /\bstatus of my\b/i,
        ],
        answer: null,
    },
