...
function createAuditor({ llm, axios, fastapiUrl, budget, router, injection,
    ...
    async function handleMessage(p) {
        ...
        //  • Product image captured at checkout/dispatch (Customer DB) — both channels.
        //  • Delivery photo (Delivery DB) — online orders only.
        ...
        const dbsChecked = channel === 'online'
        ...
        if (!checkoutImage && !deliveryImage) {
            return {
                reply: channel === 'online'
                    ? 'To verify an online order I pull the product image we recorded at dispatch and the ' +
                      'delivery photo. I couldn\'t find either for that transaction — please share your ' +
                      'transaction ID so I can locate them and check whether the seal was intact.'
                    : 'I couldn\'t find the product image captured at checkout for that transaction. Please ' +
                      'share your transaction ID so I can pull it and check whether the seal/item was intact.',
                intent: parsed.intent,
                decision: 'NEEDS_REVIEW',
        ...
        // Call the High-tier vision endpoint on FastAPI with channel + both images.
        let vision = null;
        const t0 = Date.now();
        try {
        ...
        const channelNote = channel === 'online'
