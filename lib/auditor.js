...
function createAuditor({ llm, axios, fastapiUrl, budget, router, injection,
    ...
    async function handleMessage(p) {
        ...
        // Determine the purchase channel (Scenario 1 = offline, Scenario 2 = online).
        let channel = channelHint || detectChannel(message);

        // STAGE 3 — Data retrieval.
        //  • customerImage: the photo the customer uploads NOW (current condition) — primary evidence.
        //  • checkoutImage: product image captured at checkout/dispatch (Customer DB) — cross-check.
        //  • deliveryImage: delivery photo (Delivery DB) — online orders only.
        ...
        if (transactionId != null && (channel === 'online' || channel === null)) {
            const drec = await getDeliveryImage(transactionId).catch(() => null);
            if (drec && drec.image_b64) {
                deliveryImage = drec.image_b64;
                if (channel === null) channel = 'online';   // a delivery photo implies an online order
            }
        }
        if (!channel) channel = 'offline';                  // default: in-store purchase

        const claimType = parsed.issue || 'damaged';
        const isSeal    = claimType === 'seal';
        const dbsChecked = customerImage
            ? 'the photo you uploaded'
            : channel === 'online'
                ? 'the product image from our records and the delivery photo'
                : 'the product image captured at checkout';
        ...
        // ── Anti-fraud gate: the complained product must be in the user's purchase
        // history. Recognise its MK-ID (from the uploaded photo or a provided id)
        // and cross-reference. Reject before spending the High tier if it fails.
        let purchaseContext = null;
        if (userId && (customerImage || mkId)) {
            let pv = null;
            try {
                const pr = await axios.post(`${fastapiUrl}/audit/verify-purchase`, {
                    user_id: userId, transaction_id: transactionId || null, mk_id: mkId || null, image_b64: customerImage, complaint_text: message,
                }, { timeout: 12000 });
                pv = pr.data;
            } catch (err) {
                console.warn('Purchase verification unavailable:', err.message);
            }
            if (pv && pv.status === 'REJECTED') {
                return {
                    reply: pv.message + ' If you believe this is a mistake, please contact our support team with your order ID.',
                    intent: parsed.intent,
                    decision: 'DENIED',
                    verification: { purchase: pv, channel, claim_type: claimType },
                    routing,
                    budget: snap2,
                    injection: { stage: 0, blocked: false },
                };
            }
            if (pv) purchaseContext = pv;   // APPROVED or REVIEW → proceed to the damage check
        }
        ...
        if (visDec.tier !== 'high') {
            ...
            return {
                reply: 'Thanks — I\'ve logged your ' + parsed.intent + ' request' +
                       ...
                       `will review ${dbsChecked} and follow up. ` + RETURN_POLICY_TEXT,
                intent: parsed.intent,
                decision: 'NEEDS_REVIEW',
                verification: { degraded: true, reason: visDec.reason, channel },
                routing,
                budget: charge.snapshot,
                injection: { stage: 0, blocked: false },
            };
        ...
        // We need at least one image to run the High-tier step.
        if (!customerImage && !checkoutImage && !deliveryImage) {
            return {
                reply: 'To process your refund I need to see the product. Please upload a clear, well-lit ' +
                       'photo of the item so I can check whether it\'s damaged' +
                       (transactionId ? '.' : ', or share your transaction ID so I can pull the image saved from your purchase.'),
                intent: parsed.intent,
                decision: 'NEEDS_REVIEW',
                verification: { reason: 'NO_IMAGE', channel },
                routing,
                budget: snap2,
                injection: { stage: 0, blocked: false },
            };
        ...
        // Call the High-tier vision endpoint on FastAPI with the uploaded photo + records.
        let vision = null;
        const t0 = Date.now();
        try {
            const r = await axios.post(`${fastapiUrl}/audit/verify-claim`, {
                ...
                channel,
                customer_image_b64:  customerImage,
                checkout_image_b64:  checkoutImage,
                delivery_image_b64:  deliveryImage,
        ...
        if (!vision) {
            return {
                ...
                intent: parsed.intent,
                decision: 'NEEDS_REVIEW',
                verification: { reason: 'VISION_UNAVAILABLE', channel },
                routing,
                budget: charge.snapshot,
                injection: { stage: 0, blocked: false },
            };
        ...
        const subject  = isSeal ? 'seal' : (claimType.replace('_', ' '));
        const channelNote = customerImage
            ? ' I checked the product photo you uploaded.'
            : channel === 'online'
                ? ' I checked both the product image from our records and the delivery photo.'
                : ' I checked the product image captured at checkout.';
        let decision, reply;
        if (vision.claim_supported === true) {
            decision = 'APPROVED';
            reply = `Thanks for your patience.${channelNote} The visual evidence supports your ` +
                    `"${subject}" claim (confidence ${Math.round((vision.confidence || 0) * 100)}%): ` +
                    `${vision.finding || 'the item/seal does not appear intact.'} ` +
            ...
            decision = 'DENIED';
            reply = `I reviewed the available images.${channelNote} The visual evidence does not match your ` +
                    `"${subject}" claim (confidence ${Math.round((vision.confidence || 0) * 100)}%): ` +
                    `${vision.finding || 'the item/seal appears intact.'} So I\'m unable to approve this ` +
                    `${parsed.intent} automatically. If you believe this is a mistake, a store associate can take a closer look.`;
        } else {
            decision = 'NEEDS_REVIEW';
            reply = `I\'ve reviewed the available images but the evidence is inconclusive` +
                    `${vision.finding ? ` (${vision.finding})` : ''}. I\'ve forwarded your ` +
